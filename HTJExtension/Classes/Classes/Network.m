...
- (instancetype)_init
{
    ...
    if (self) {
        ...
        // [self.requestSerializer setValue:[HTLocalConfigModel sharedConfigModel].idfa forHTTPHeaderField:@"mobiledevicecodeidfa"];
        [self.requestSerializer setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
        // [self.requestSerializer setValue:[HTUserModel shared].currentCookie forHTTPHeaderField:@"Set-Cookie"];
...
- (ConfigToDictionary)header {
    return ^(NSDictionary *header) {
        self.headerDictionary = [NSMutableDictionary dictionaryWithDictionary:header];
        return self;
    };
}
...
+ (void)cancelRequestWithUrl:(NSString *)url {
    ...
    [Network.netConfig.httpManager.dataTasks.rac_sequence.signal subscribeNext:^(NSURLSessionDataTask * _Nullable x) {
        NSLog(@"log::url - %@", x.currentRequest.URL.absoluteString);
        if ([x.currentRequest.URL.absoluteString isEqualToString:url]) {
            [x cancel];
            return;
        }
    }];
